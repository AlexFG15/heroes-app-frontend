...
import { FavoriteHeroContext } from "../context/FavoriteHeroeContext";
    ...
    <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
      <div className="relative h-64 ">
        ...
        <div className="absolute top-3 left-3 flex items-center gap-2">
          <div
            className={`w-3 h-3 rounded-full ${heroe.status === "Active" ? "bg-green-500" : "bg-red-500"} `}
          />
          <Badge
            variant="secondary"
        ...
        >
          <Heart
            className={`h-4 w-4 ${isFavorite(heroe) ? "fill-red-500 text-red-500" : "text-gray-500"} `}
          />
        </Button>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-xs text-gray-500 pt-2 border-t">
          Primera aparaición: {heroe.firstAppearance}
        </div>
      </CardContent>
    </Card>
  );
